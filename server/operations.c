...
int ems_reserve(unsigned int event_id, size_t num_seats, size_t* xs, size_t* ys) {
  ...
  printf("Sucess Reserved seats in event \n");
  pthread_mutex_unlock(&event->mutex);
  return 0;
}

int ems_show( unsigned int event_id, unsigned int* seats) {
  if (event_list == NULL) {
    fprintf(stderr, "EMS state must be initialized\n");
    return 1;
  }
  ...
  unsigned int* current_seat = seats;
  for (size_t i = 1; i <= event->rows; i++) {
    for (size_t j = 1; j <= event->cols; j++) {
      unsigned int seat_data = event->data[seat_index(event, i, j)];
      *current_seat = seat_data;
      current_seat++;
    }
  }
...
int ems_list_events(unsigned int* event_ids, int max_ids) {
  if (event_list == NULL) {
    fprintf(stderr, "EMS state must be initialized\n");
    return 1;
  }
  ...
  while (1) {
    ...
    if (index >= max_ids) {
      fprintf(stderr, "Not enough space in the array\n");
      pthread_rwlock_unlock(&event_list->rwl);
      return 1;
    }

    // Store event ID in the array
  ...
  pthread_rwlock_unlock(&event_list->rwl);
  return index; // Return the number of stored event IDs
}

size_t get_event_rows(unsigned int event_id){
  struct ListNode *current;
  for (current = event_list->head; current != NULL; current = current->next) {
      if (current->event->id == event_id) {
          return current->event->rows;
        }
      }
      return 0;
  }

size_t get_event_cols(unsigned int  event_id){
  struct ListNode *current;
  for (current = event_list->head; current != NULL; current = current->next) {
      if (current->event->id == event_id) {
          return current->event->cols;
        }
      }
    return 0;
  }
